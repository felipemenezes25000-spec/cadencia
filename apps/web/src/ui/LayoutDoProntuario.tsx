...
export interface SecaoDoLayout {
  ...
  /** false = ainda segue o padrao da clinica. */
...
export interface LayoutDoProntuarioProps {
  ...
  /** Lista VAZIA volta ao padrao da clinica. */
  readonly aoSalvar: (itens: readonly ItemSalvo[]) => Promise<void>;
}

/**
 * Cada medico arruma o proprio prontuario.
 *
 * `clin.record_layout_item` existia no banco desde a Fase 1 — com indice,
 * unicidade por (profissional, secao) e tudo — e nunca teve uma linha. Todo
 * medico via as secoes na ordem da clinica, e o ortopedista abria
 * "antecedentes ginecologicos" no topo em toda consulta.
 *
 * Setas e nao arrastar: uma lista de seis a dez secoes reordena mais rapido com
 * dois cliques do que com drag, funciona no teclado sem trabalho extra e nao
 * quebra no celular. Arrastar seria mais bonito e menos util.
 */
    ...
    <section className="grid gap-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-text-muted">
          {jaPersonalizado
            ? 'Esta e a sua ordem. A clinica pode mudar o padrao dela sem afetar isto.'
            : 'Seguindo a ordem da clinica. Mexer aqui vale so para voce.'}
        </p>
        {jaPersonalizado && (
          <Botao variante="fantasma" tamanho="sm"
            iconeEsquerda={ArrowCounterClockwise}
            carregando={salvando}
            // Lista vazia apaga a personalizacao: a partir dai o medico volta a
            // receber as mudancas que a clinica fizer no padrao.
            onClick={() => { void salvar([]); }}>
            Voltar ao padrao da clinica
          </Botao>
        )}
      </div>
      ...
      {nenhumaVisivel && (
        <p role="alert" className="text-sm text-danger">
          Deixe <strong>pelo menos uma</strong> secao visivel — a ficha sem
          nenhuma e uma tela de atendimento em branco.
        </p>
      )}

      <div>
        <Botao
          // Salvar sem mudanca gravaria um layout identico ao padrao, e a partir
          // dai o medico deixaria de receber as mudancas da clinica.
          disabled={!mexeu || nenhumaVisivel}
          carregando={salvando}
          onClick={() => { void salvar(ordem.map((s, i) => ({
            sectionId: s.sectionId,
            // Renumerado de 1 a N: buraco de ordinal deixaria a proxima
            // reordenacao ambigua.
            ordinal: i + 1,
            visible: s.visible,
