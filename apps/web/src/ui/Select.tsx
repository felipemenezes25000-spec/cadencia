...
const triggerBase = [
  'flex h-9 w-full items-center justify-between gap-2 rounded-md border bg-surface px-3',
  'text-sm text-text',
...
function SelectItem({ value, label, disabled = false }: SelectOption) {
  return (
    <RadixSelect.Item
      ...
      disabled={disabled}
      className={cn(
        'relative flex cursor-pointer select-none items-center rounded-sm px-3 py-1.5 pr-8 text-sm text-text',
        'outline-none',
    ...
    <div className={cn('flex flex-col gap-1', className)}>
      ...
      >
        ...
        <RadixSelect.Portal>
          <RadixSelect.Content
            className={cn(
              'z-[var(--z-popover)] overflow-hidden rounded-md border border-line bg-surface shadow-elev-2',
