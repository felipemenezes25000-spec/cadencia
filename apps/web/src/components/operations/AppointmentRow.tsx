...
function Sinal({ children, tone = 'neutral' }: {
  ...
  return (
    <span className={cn(
      ...
      tone === 'warning'
        ? 'border-warn/15 bg-warn-soft/60 text-warn'
        : tone === 'info'
          ? 'border-accent/15 bg-accent-soft/55 text-accent'
...
export function AppointmentRow({
  ...
  return (
    ...
    >
      {estadoVivo ? (
        <span className={cn(
          'absolute inset-y-4 left-0 w-[3px] rounded-r-full shadow-[0_0_14px_currentColor]',
          appointment.status === 'atendendo' ? 'bg-accent text-accent' : 'bg-warn text-warn',
        )} aria-hidden />
      ) : null}

      <div className="col-start-1 row-span-2 self-start pt-0.5 md:row-span-1 md:self-auto md:pt-0">
        <time className="block font-mono text-[12px] font-bold tracking-[-0.02em] text-text tabular-nums">
          {horario}
        </time>
      ...
      <button
        ...
        className="group/patient col-start-2 row-start-1 flex min-w-0 items-center gap-3 rounded-xl text-left outline-none focus-visible:ring-2 focus-visible:ring-accent/55 focus-visible:ring-offset-4 md:col-start-auto md:row-start-auto"
      >
        <Avatar nome={appointment.displayName} tamanho="md" />
        <span className="min-w-0">
          <span className="block truncate text-[13px] font-bold tracking-[-0.018em] text-text transition-colors group-hover/patient:text-accent sm:text-sm">
      ...
      <span className={cn(
        'hidden text-[11px] font-semibold md:flex md:items-center md:gap-1.5',
        appointment.status === 'aguardando' ? 'text-warn' : appointment.status === 'atendendo' ? 'text-accent' : 'text-text-faint',
      )}>
      ...
      <Botao
        ...
        className="col-start-3 row-start-1 min-w-[108px] md:col-start-auto md:row-start-auto md:w-[132px]"
      >
        {action.label}
      </Botao>
...
function MenuItem({ icon: Icone, label, onSelect }: {
  ...
  return (
    <DropdownMenu.Item
      onSelect={onSelect}
      className="flex cursor-pointer items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm font-medium text-text outline-none data-[highlighted]:bg-surface-subtle"
    >
      <span className="grid size-7 place-items-center rounded-lg bg-surface-sunken text-text-faint">
