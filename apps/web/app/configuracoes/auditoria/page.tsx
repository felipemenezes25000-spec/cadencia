...
/** Data local da clínica — a trilha é lida no fuso de quem responde por ela. */
...
function AuditoriaInner() {
  ...
  useEffect(() => {
    ...
    void apiFetch<{ itens: EventoDaTrilha[] }>(`/v1/auditoria?${q.toString()}`,
      ...
      .catch((e: unknown) => {
        if (!vivo) return;
        // A API já diz POR QUE recusou: `motivo: 'mfa_exigida'` em `dados`.
        // Jogar isso fora e mostrar "não foi possível" faz um administrador com
        // o papel certo achar que a tela quebrou — quando o que falta é um
        // passo que ele mesmo pode dar.
        const motivo = e instanceof ApiError ? e.dados['motivo'] : undefined;
        setErro(motivo === 'mfa_exigida'
          ? 'A trilha exige verificação em duas etapas. Ative a sua em Configurações e entre de novo — a auditoria é o registro de quem viu o que, e ler isso sem segundo fator seria uma porta aberta.'
          : e instanceof ApiError && e.status === 403
            ? 'Seu perfil não lê a trilha de auditoria. Só administração e direção técnica leem.'
            : 'Não foi possível ler a trilha.');
      });
    return () => { vivo = false; };
    ...
    <div className="cadencia-page grid gap-5">
      ...
      <div className="grid gap-3 sm:grid-cols-4">
        ...
        <label className="grid gap-1.5 text-sm">
          ...
          <select value={outcome} onChange={(e) => { void setOutcome(e.target.value); }}
            ...
            {/* `negado` é o que a auditoria procura primeiro: alguém tentou e
                o sistema recusou. Sucesso é ruído quando se investiga acesso. */}
      ...
      {itens !== null && itens.length === 0 && (
        <p className="text-sm text-text-muted">
          Nenhum evento no período com esses filtros.
        </p>
      )}
              ...
              {itens.map((e) => (
                <tr key={e.eventId} className="border-t border-line align-top">
                  <td className="py-2 whitespace-nowrap tabular-nums text-text-muted">
                    {/* Fuso da CLÍNICA, sempre: a trilha é lida por quem
                        responde por ela, que está no horário da unidade. */}
                  ...
                  <td className="py-2">
                    ...
                    {e.atorKind !== 'user' && (
                      ...
                      // online agem sem pessoa por trás, e confundi-los com um
                      // usuário faria a auditoria acusar quem não fez nada.
