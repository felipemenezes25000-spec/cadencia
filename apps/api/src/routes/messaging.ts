...
export async function messagingRoutes(app: FastifyInstance): Promise<void> {
  ...
  r.get('/v1/conversations', {
    schema: {
      ...
      response: {
        200: z.object({ itens: z.array(ConversationSchema), nextCursor: z.string().nullable() }),
      },
    },
  ...
  r.get('/v1/conversations/:id/messages', {
    schema: {
      ...
      response: {
        200: z.object({ itens: z.array(MessageSchema), nextCursor: z.string().nullable() }),
      },
    },
    ...
    const { rows } = await tx.query<{
      ...
      channel: string; body_text: string | null; body_media_key: string | null;
      status: string; created_at: string;
    }>(
      `SELECT m.id AS message_id, m.conversation_id, m.direction, m.channel,
  ...
  r.get('/v1/conversations/:id/contexto', {
      ...
      response: {
        200: z.object({
          ...
          historicoAgendamentos: z.array(z.object({
            appointmentId: z.string().uuid(), quando: z.string(), status: z.string(),
          })),
        }),
    ...
    if (conv.length === 0) return reply.code(404).send({ erro: 'nao_encontrado' as const });
    ...
    const { rows: futuros } = await tx.query<{
      ...
      `SELECT a.id, a.starts_at AS quando, u.full_name AS profissional,
              pr.nome AS procedimento, a.status::text AS status
    ...
    const { rows: passados } = await tx.query<{ id: string; quando: Date; status: string }>(
  ...
  r.post('/v1/conversations/:id/messages', {
    schema: {
      ...
      response: {
        201: z.object({ messageId: z.string().uuid(), status: z.literal('queued') }),
      },
    },
  ...
  r.get('/v1/messaging/templates', {
    schema: {
      ...
      response: { 200: z.object({ itens: z.array(TemplateSchema) }) },
    },
    ...
    const { rows } = await tx.query<{
      ...
      channel: string; category: string; body_template: string;
      variables: unknown; status: string; updated_at: string;
    }>(
      `SELECT t.id AS template_id, t.name, t.channel_identity_id, t.channel, t.category,
              t.body_template, t.variables, t.status,
              to_char(t.updated_at AT TIME ZONE 'UTC',
                      'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') AS updated_at
         FROM msg.template t ${where}
  ...
  r.post('/v1/messaging/templates', {
    schema: {
      ...
      response: { 200: z.object({ templateId: z.string().uuid(), status: z.string() }) },
    },
    ...
    const channel = ciRows[0]!.channel;

    if (b.templateId !== undefined) {
      const atualizacao = await tx.query(
        `UPDATE msg.template
            SET channel_identity_id = $2,
                channel = $3,
                name = $4,
                category = $5,
                body_template = $6,
          ...
          WHERE id = $1`,
        [b.templateId, b.channelIdentityId, channel, b.name, b.category,
         b.bodyTemplate, JSON.stringify(b.variables)],
      );
      if (atualizacao.rowCount === 0) erroDominio('template_nao_encontrado', 404);
    ...
    const { rows } = await tx.query<{ template_id: string }>(
      ...
      [b.channelIdentityId, channel, b.name, b.category,
       b.bodyTemplate, JSON.stringify(b.variables)],
    );
  ...
  r.get('/v1/messaging/automations', {
    schema: { response: { 200: z.object({ itens: z.array(AutomationRuleSchema) }) } },
  }, rota('messaging.automation.write', async (tx) => {
    const { rows } = await tx.query<{
      rule_id: string; trigger: z.infer<typeof AutomationTriggerSchema>;
      channel_identity_id: string; template_id: string;
      timing_offset_minutes: string; active: boolean;
      channel: 'whatsapp' | 'sms' | 'email'; updated_at: string;
    }>(
      `SELECT r.id AS rule_id, r.trigger, r.channel_identity_id, r.template_id,
  ...
  r.put('/v1/messaging/automations', {
    ...
    const b = req.body as {
      rules: Array<{
        ruleId?: string; channelIdentityId: string;
        trigger: z.infer<typeof AutomationTriggerSchema>;
        templateId: string; timingOffsetMinutes: number;
        active: boolean; channel: 'whatsapp' | 'sms' | 'email';
      }>;
    };

    let saved = 0;
    for (const rule of b.rules) {
      // Identidade, template e canal formam um conjunto. Sem esta checagem era
      // possível salvar regra SMS apontando para template WhatsApp; o worker só
      // descobria o erro minutos depois, na hora do disparo.
      const compativel = await tx.query(
        `SELECT 1
           FROM msg.channel_identity ci
           JOIN msg.template t
             ON t.tenant_id = ci.tenant_id
            AND t.channel_identity_id = ci.id
          WHERE ci.id = $1
            AND t.id = $2
            AND ci.channel = $3
            AND t.channel = $3`,
        [rule.channelIdentityId, rule.templateId, rule.channel],
      );
      if (compativel.rows.length === 0) {
        erroDominio('automacao_canal_template_incompativel', 422);
      }
        ...
        const atualizado = await tx.query(
          `UPDATE msg.automation_rule
              SET channel_identity_id = $2,
                  ...
                  updated_at = clock_timestamp()
            WHERE id = $1`,
          [rule.ruleId, rule.channelIdentityId, rule.trigger, rule.templateId,
           rule.timingOffsetMinutes, rule.active, rule.channel],
        );
        if (atualizado.rowCount === 0) erroDominio('automacao_nao_encontrada', 404);
      } else {
        await tx.query(
