...
async function semearLancamento(): Promise<{ tenantId: string; entryId: string; userId: string }> {
  ...
  try {
    ...
    const userId = uuidv7();
    const profId = uuidv7();
    // Slug vai como parametro proprio: reaproveitar $1 como uuid e como texto
    // faz o Postgres deduzir tipos conflitantes para o mesmo parametro.
    await a.query(
...
describe('gerar link de pagamento', () => {
  it('cria o link no provedor e grava a URL', async () => {
    ...
    // Narrowing explicito: o retorno e uniao discriminada, e so o ramo 'criado'
    // tem url. Ler .url sem estreitar e o que o typecheck pega e o vitest nao.
  ...
  it('rodar de novo nao cria segundo link — o outbox entrega ao menos uma vez', async () => {
    ...
    // Entrega "ao menos uma vez" e a garantia do outbox, nao "exatamente uma".
    // Sem idempotencia aqui, uma reentrega geraria uma SEGUNDA cobranca para o
  ...
  it('lancamento inexistente nao explode a fila', async () => {
    const r = await gerarLinkDePagamento(
      ...
      createFakePaymentProvider());
    // Job que lanca excecao volta para a fila e tenta de novo para sempre.
    // Lancamento que nao existe nunca vai existir: e falha terminal.
