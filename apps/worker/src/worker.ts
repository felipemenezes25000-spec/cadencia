...
export async function startWorker(): Promise<PgBoss> {
  const boss = new PgBoss({
    ...
    ...(process.env['DATABASE_JOBS_PASSWORD'] === undefined ? {} : { password: process.env['DATABASE_JOBS_PASSWORD'] }),
    schema: 'pgboss', migrate: false,
  });
  await boss.start();
  const { messaging, sms, email, calendar } = workerProviders();

  await boss.work(FILA_RASCUNHOS, async () => {
    const r = await autoFinalizeStaleDrafts({ limiteDias: 7 });
    process.stdout.write(`[worker] auto-finalize: ${r.finalizados}/${r.examinados} (falhas: ${r.falhas})\n`);
  });
  ...
  await boss.work(FILA_EXPURGO, async () => {
    ...
    process.stdout.write(`[worker] expurgo: ${r.valoresExpurgados} valores, ${r.anexosExpurgados} anexos, ${r.objetosApagados} objetos apagados, ${r.falhas} falhas\n`);
    if (r.falhas > 0) process.stderr.write(`[worker] EXPURGO INCOMPLETO: ${r.falhas} objetos permanecem no armazenamento\n`);
  });

  await boss.work(FILA_OUTBOX, async () => {
    const r = await dispatchOutbox(boss);
    if (r.dispatched > 0 || r.errors > 0) process.stdout.write(`[worker] outbox: ${r.dispatched} despachados, ${r.errors} erros\n`);
  });
  ...
  await boss.work(FILA_ENVIO_MSG, async (jobs) => {
    for (const job of jobs) {
      ...
      process.stdout.write(`[worker] send-message: ${r.messageId} -> ${r.status}\n`);
      if (r.status === 'retryable') throw new Error(r.detail);
      if (r.status === 'indeterminate') process.stderr.write(`[worker] MENSAGEM INDETERMINADA: ${r.messageId}: ${r.detail}\n`);
    }
  });
  ...
  await boss.work(FILA_ENVIO_LEMBRETE, async (jobs) => {
    for (const job of jobs) {
      ...
      process.stdout.write(`[worker] reminder: ${data.appointmentId}/${data.ruleId} -> ${r.status}\n`);
      if (r.status === 'retryable') throw new Error(r.detail);
      if (r.status === 'indeterminate') process.stderr.write(`[worker] REMINDER INDETERMINADO: ${data.appointmentId}/${data.ruleId}: ${r.detail}\n`);
    }
  });
  ...
  await boss.work(FILA_LEMBRETES, async () => {
    ...
    process.stdout.write(`[worker] reminders: ${r.scheduled} agendados, ${r.skipped} pulados\n`);
  });

  await boss.work(FILA_SELO, async () => {
    const r = await selarTrilha();
    process.stdout.write(`[worker] selo ${r.dia}: ${r.tenantsSelados} selados, ${r.jaSelados} ja selados, ${r.adiados} adiados, ${r.falhas} falhas\n`);
    const vigia = await vigiarSelo();
    if (vigia.atrasado) process.stderr.write(`[worker] VIGIA DO SELO: ${vigia.status} — ultima execucao ${vigia.ultimaExecucao ?? 'nunca'}\n`);
  });
  ...
  await boss.work(FILA_CALENDAR_SYNC, async (jobs) => {
    for (const job of jobs) {
      ...
      process.stdout.write(`[worker] calendar-sync: ${r.usersProcessed} usuarios, ${r.eventsSynced} eventos, ${r.failures} falhas\n`);
      if (r.failures > 0 && d.force === true) throw new Error(`calendar-sync falhou em ${r.failures} conexao(oes)`);
    }
  });
...
async function main(): Promise<void> {
  ...
  for (const sinal of ['SIGTERM', 'SIGINT'] as const) {
    process.once(sinal, () => { void (async () => { await boss.stop(); await closePools(); process.exit(0); })(); });
  }
}
