...
/**
 * Calcula o instante UTC de envio de lembrete/automação.
 *
 * O timing_offset_minutes é RELATIVO ao horário LOCAL da clínica, NÃO ao UTC.
 * Motivo: consulta as 8h em America/Sao_Paulo com offset -1440 deve disparar
 * as 8h do dia anterior no fuso local, ou seja 11:00 UTC — e não as 5:00 UTC
 * que é o que daria se subtraísse 1440 min do instante UTC puro.
 *
 * Algoritmo:
 * 1. Converte starts_at UTC para horário local da clínica.
 * 2. Aplica o offset em minutos no horário local.
 * 3. Converte de volta para UTC.
 *
 * Usa Intl.DateTimeFormat para obter o offset real do fuso naquele instante,
 * respeitando horário de verão onde aplicável.
 */
...
function utcOffsetMinutesAt(instantMs: number, timezone: string): number {
  ...
  // Reconstrói o instante "como se fosse UTC" a partir das partes locais
  const localAsUtc = Date.UTC(
    get('year'), get('month') - 1, get('day'),
    get('hour') === 24 ? 0 : get('hour'), get('minute'), get('second'),
  );

  // A diferença é o offset: local - UTC
...
export function computeReminderInstant(
  ...
  // 2. Horário local da consulta em ms (como se fosse UTC)
  const localMs = startsMs + tzOffsetAtStart * 60_000;

  // 3. Aplica o offset da automação no horário local
