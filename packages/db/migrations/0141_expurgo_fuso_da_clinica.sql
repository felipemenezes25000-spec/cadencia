-- 0141_expurgo_fuso_da_clinica.sql
-- Forward-only: nao existe down migration. Para desfazer, escreva a proxima.
-- Este arquivo roda dentro de UMA transacao. Nada de CREATE INDEX CONCURRENTLY.
--
-- O lint de DDL (invariante 08) reprovou `clin.purge_expired`: ela calculava a
-- data-limite com `current_date`, que e a data do SERVIDOR.
--
-- A regra existe porque data pertence a clinica, nao ao banco (decisao
-- irreversivel 10). Uma unidade em Rio Branco fecha o dia cinco horas depois de
-- sa-east-1; corte no fuso errado expurga um dia a mais de acervo clinico, e
-- expurgo nao tem desfazer. Em vinte anos um dia parece pouco — e e por parecer
-- pouco que a regra e automatica: ninguem precisa julgar se importa.
--
-- `min(...)` entre as unidades do tenant: o corte mais CONSERVADOR. Se as
-- unidades estao em fusos diferentes, vale a que ainda nao virou o dia.
...
BEGIN
  ...
  IF v_corte IS NULL THEN
    RETURN QUERY SELECT 0::bigint, 0::bigint, ARRAY[]::uuid[], NULL::date;
    RETURN;
  END IF;
...
GRANT SELECT ON app.clinic TO clin_purger;
CREATE POLICY purger ON app.clinic AS PERMISSIVE FOR SELECT TO clin_purger
