// Monta o plano consolidado da Fase 2 a partir dos 10 blocos em
// docs/superpowers/plans/fase2/ + o cabecalho de contexto.
//
//   node tools/montar-plano-fase2.mjs
//
// Gera: docs/superpowers/plans/2026-08-04-fase2-a-conversa-e-o-caixa-plano.md

import { readFileSync, writeFileSync, readdirSync } from 'node:fs';
import { join } from 'node:path';

const RAIZ = join(import.meta.dirname, '..');
const DIR_BLOCOS = join(RAIZ, 'docs/superpowers/plans/fase2');
const SAIDA = join(RAIZ, 'docs/superpowers/plans/2026-08-04-fase2-a-conversa-e-o-caixa-plano.md');

const CABECALHO = `# Cadência — Fase 2 ("A conversa e o caixa"): Plano de Implementação
...
const PARTES = [
  { titulo: 'Parte I — Fundação assíncrona', blocos: ['01-events-outbox'] },
  { titulo: 'Parte II — Mensageria', blocos: ['02-messaging-modelo', '03-whatsapp-provider', '04-automacoes'] },
  { titulo: 'Parte III — Financeiro', blocos: ['05-pagamentos-modelo', '06-payment-link-conciliacao'] },
  { titulo: 'Parte IV — API e Worker', blocos: ['07-api-worker'] },
  { titulo: 'Parte V — Telas', blocos: ['08-telas-conversas', '09-telas-financeiro'] },
  { titulo: 'Parte VI — Integração e gates', blocos: ['10-integracao-gate'] },
];

let conteudo = CABECALHO;

for (const parte of PARTES) {
  conteudo += `## ${parte.titulo}\n\n`;
  for (const chave of parte.blocos) {
    const arquivo = join(DIR_BLOCOS, chave + '.md');
    let texto = readFileSync(arquivo, 'utf8');
    // Remove reconciliation header comments
    texto = texto.replace(/^<!--[\s\S]*?-->\s*\n*/m, '');
    conteudo += texto.trimEnd() + '\n\n';
  }
}

writeFileSync(SAIDA, conteudo, 'utf8');

const tarefas = [...conteudo.matchAll(/^### Task (\S+?):/gm)];
console.log(`plano consolidado gravado em ${SAIDA}`);
console.log(`${tarefas.length} tarefas, faixa ${tarefas[0]?.[1]} a ${tarefas.at(-1)?.[1]}`);
